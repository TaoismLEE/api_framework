...
@ddt.ddt
class Isolate_Demo_API(unittest.TestCase):
    """XXX系统"""
    def setUp(self):
    ...
    def test_isolate_api(self, data):
        """
        1. 当所有接口均是独立的（无相互依赖），则在该方法体内直接调用call_api方法就行了
        2. 当接口之间有依赖关系时，即测试流程接口时：
            数据驱动从第1个接口往下执行，因此通过if判断当前正在执行的接口
            执行后续依赖接口，可能会用到之前接口的返回数据如token或者从数据库拿数据，则可以在每个接口执行前重构data参数，eg.下面示例，获取登录接口返回的token
        """
